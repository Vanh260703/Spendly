...
import {
  Button, Card, CardTitle, EmptyState, ErrorState, Field, Input, Modal, MoneyInput, Progress,
  Select, Skeleton,
} from '@/components/ui';
...
export default function DebtsPage() {
  ...
  const [form, setForm] = useState<{
    ...
    minPayment: number | '';
    dueDay: string;
  }>({ name: '', principal: '', interestRate: '', minPayment: '', dueDay: '15' });
  ...
  const guiTao = (e: React.FormEvent) => {
    ...
    tao.mutate(
      ...
      },
      {
        onSuccess: () => {
          setMoTao(false);
          setForm({ name: '', principal: '', interestRate: '', minPayment: '', dueDay: '15' });
        },
      },
    );
  };
    ...
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      {isLoading ? (
        ...
        <Card>
          <EmptyState
            ...
            description="Tuyệt vời — bạn đang không nợ ai cả"
          />
        </Card>
      ) : (
        <>
              ...
              <div className="mb-2 flex items-start justify-between">
                <div>
                  ...
                  <p className="muted text-xs">
                    Lãi {d.interestRate}%/năm · trả tối thiểu {formatMoney(d.minPayment)}/tháng
                    · hạn ngày {d.dueDay}
                  </p>
                </div>
                <Button variant="secondary" size="sm" onClick={() => setTraCho(d)}>
                  Trả nợ
                </Button>
              </div>
          ...
          <Card>
            ...
            {plan && (
              <div className="mt-3 space-y-2">
                {plan.debtFreeDate ? (
                  <p className="text-sm">
                    Hết nợ vào <strong>{formatDate(plan.debtFreeDate)}</strong> ({plan.months}{' '}
                    tháng) · tổng lãi{' '}
                  ...
                  <p className="text-sm text-expense">
                    Mức trả hiện tại không đủ để tất toán — lãi phát sinh nhanh hơn tiền trả.
                    Hãy tăng khoản trả thêm.
                  </p>
                )}
                ...
                <ol className="space-y-1.5">
                  {plan.order.map((o, i) => (
                    <li key={o.debtId} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="tabular text-xs text-expense">
                        +{formatMoney(o.totalInterest)} lãi
                      </span>
                    </li>
                  ))}
                </ol>
      ...
      <Modal open={moTao} onClose={() => setMoTao(false)} title="Thêm khoản nợ">
        <form onSubmit={guiTao} className="space-y-4">
          ...
          <Button type="submit" loading={tao.isPending} className="w-full">
            Thêm
          </Button>
        </form>
      </Modal>
      ...
      <Modal open={!!traCho} onClose={() => setTraCho(null)} title={`Trả nợ "${traCho?.name}"`}>
        <form
          onSubmit={(e) => {
            ...
            if (!traCho) return;
            tra.mutate(
              { id: traCho.id, amount: Number(soTien) },
              { onSuccess: () => { setTraCho(null); setSoTien(''); } },
            );
          }}
          className="space-y-4"
        >
          ...
          <Button type="submit" loading={tra.isPending} className="w-full">
            Ghi nhận
          </Button>
        </form>
      </Modal>
