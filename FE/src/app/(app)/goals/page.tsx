...
import {
  Button, Card, EmptyState, ErrorState, Field, Input, Modal, MoneyInput, Progress, Select,
  Skeleton,
} from '@/components/ui';
...
export default function GoalsPage() {
  ...
  }>({ name: '', targetAmount: '', horizon: 'short', deadline: '', monthlyContribution: '' });
  ...
  const guiTao = (e: React.FormEvent) => {
    ...
    tao.mutate(
      ...
      },
      {
        onSuccess: () => {
          setMoTao(false);
          setForm({ name: '', targetAmount: '', horizon: 'short', deadline: '', monthlyContribution: '' });
        },
      },
    );
  };
    ...
    <div className="mx-auto max-w-3xl space-y-4">
            ...
            <div className="mb-2 flex items-start justify-between gap-2">
              <div>
                <p className="font-medium">
                  {g.name}
                  {g.status === 'achieved' && (
                    <span className="ml-2 text-sm text-brand">✓ đã đạt</span>
                  )}
                </p>
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  if (confirm(`Xóa mục tiêu "${g.name}"?`)) xoa.mutate(g.id);
                }}
      ...
      <Modal open={!!napVao} onClose={() => setNapVao(null)} title={`Nạp vào "${napVao?.name}"`}>
        ...
        >
          ...
          <Button type="submit" loading={nap.isPending} className="w-full">
            Nạp
          </Button>
        </form>
      </Modal>
