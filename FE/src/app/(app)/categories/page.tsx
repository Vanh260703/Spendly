...
export default function CategoriesPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <Modal open={mo} onClose={dong} title={editing ? `Sửa "${editing.name}"` : 'Thêm danh mục'}>
        {/* `key` buộc remount mỗi lần đổi mục đang sửa — CategoryForm khởi tạo state từ
            `editing` MỘT LẦN lúc mount, không đồng bộ lại qua effect */}
        <CategoryForm key={editing?.id ?? 'new'} editing={editing} defaultType={loaiMoi} onDone={dong} />
      </Modal>
    </div>
  );
}
