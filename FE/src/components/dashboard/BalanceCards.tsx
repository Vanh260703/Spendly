...
import { formatMoneyAnHien, formatPercent } from '@/lib/format';

export function BalanceCards({ period, anSoDu }: { period: string; anSoDu: boolean }) {
  const balance = useBalance();
  const summary = useSummary({ period });
  const tien = (n: number, opts?: { sign?: boolean }) => formatMoneyAnHien(n, anSoDu, opts);
  ...
  if (balance.isError || summary.isError) {
    return (
      <Card>
        <ErrorState
          message={((balance.error ?? summary.error) as ApiError)?.message}
          onRetry={() => {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Card className="bg-brand text-white">
        ...
        <p className="tabular mt-2 text-2xl font-bold">{tien(b?.currentBalance ?? 0)}</p>
        {((b?.committedToGoals ?? 0) > 0 || (b?.owedByMe ?? 0) > 0) && (
          <p className="mt-1 flex items-center gap-1 text-xs opacity-90">
            <Lock size={12} /> Tự do tiêu: {tien(b?.freeToSpend ?? 0)}
          </p>
        )}
        {/*
          Tiền bạn bè đang nợ nằm NGOÀI tài khoản nên không cộng vào số dư — hiện riêng để
          user biết còn khoản sẽ về, chứ tuyệt đối không gộp vào "tự do tiêu": tiêu tiền
          chưa về tay là tiêu khống.
        */}
        {(b?.owedToMe ?? 0) > 0 && (
          <p className="mt-1 flex items-center gap-1 text-xs opacity-90">
            <Users size={12} /> Bạn bè nợ: {tien(b?.owedToMe ?? 0)}
          </p>
        )}
      </Card>
        ...
        <Stat
          label={
            <span className="flex items-center gap-1.5">
              <ArrowDownLeft size={14} className="text-income" /> Thu trong kỳ
            </span>
          }
          value={tien(s?.income ?? 0)}
          tone="income"
        />
      </Card>
      ...
      <Card>
        <Stat
          label={
            <span className="flex items-center gap-1.5">
              <ArrowUpRight size={14} className="text-expense" /> Chi trong kỳ
            </span>
          }
          value={tien(s?.expense ?? 0)}
          tone="expense"
        >
      ...
      <Card>
        <Stat
          label="Chênh lệch"
          value={tien(s?.net ?? 0, { sign: true })}
          tone={(s?.net ?? 0) >= 0 ? 'income' : 'expense'}
        >
