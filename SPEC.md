...
- **TLS cho Postgres bật/tắt bằng `DB_SSL`, mặc định TẮT.** Postgres quản lý (Neon, Supabase…) **chỉ** nhận TLS, còn Postgres local thì phải tắt — bản Docker/Homebrew không bật TLS, bản Ubuntu/Debian có bật nhưng bằng chứng chỉ **tự ký** nên xác minh trượt. ⚠️ Bật `DB_SSL=true` mà quên đổi `DB_HOST` sẽ chết với `Error: self-signed certificate` — câu lỗi không nhắc gì tới `DB_SSL`, đã mất công lần một lần rồi. `data-source.ts` đọc thẳng `process.env` chứ không qua `ConfigService` vì file này còn được TypeORM CLI nạp lúc chạy migration, khi chưa có container Nest nào. ⚠️ **`rejectUnauthorized: true` là cố ý** — hướng dẫn trên mạng hay để `false` cho hết lỗi, nhưng thế là tắt xác minh chứng chỉ: vẫn mã hóa mà không còn biết đầu bên kia có đúng máy chủ mình định nối tới không. Neon dùng chứng chỉ CA công cộng nên xác minh chạy bình thường. ⚠️ **Không dùng `z.coerce.boolean()` cho biến env**: `Boolean("false") === true` nên `DB_SSL=false` sẽ bị hiểu ngược thành BẬT; phải so chuỗi tường minh `v === 'true'`. Đã kiểm chứng bằng cách chạy `validateEnv` với `undefined`/`false`/`FALSE`/`0`/`true`.
