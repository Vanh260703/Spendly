...
import {
  GenerateInsightDto, ListInsightQuery, NecessityQuery,
  generateInsightSchema, listInsightQuerySchema, necessityQuerySchema,
} from './dto/ai.dto';
  ...
  generate(
    ...
    switch (dto.kind) {
      case InsightKind.HEALTH_SCORE:
        return this.ai.healthScore(user.id, dto.force);
      case InsightKind.WEEKLY:
        return this.ai.periodReport(user.id, 'week', undefined, dto.force);
      case InsightKind.MONTHLY:
        return this.ai.periodReport(user.id, 'month', undefined, dto.force);
      default:
        return this.ai.necessityReview(user.id, dto.period, undefined, dto.force);
    }
  }
}
