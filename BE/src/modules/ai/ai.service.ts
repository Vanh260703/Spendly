...
import { formatDateOnly, resolvePeriod } from '../../common/utils/period';
...
export class AiService {
  ...
  async healthScore(userId: string, force = false) {
  ...
  /**
   * Sinh insight với cache **một lớp, ở Postgres**, khóa theo `inputHash`.
   *
   ...
   *
   * ⚠️ Cố ý KHÔNG có lớp đếm lượt gọi/ngày — user tự quản lý việc đó. Lớp cache này mới là
   * chốt chặn THẬT: dữ liệu tài chính trong một kỳ không đổi liên tục, nên phần lớn lượt
   * gọi trùng đã bị chặn ở đây trước khi tới `LlmClient`.
   */
  private async sinhInsight(args: {
    ...
    if (!force) {
      ...
      if (ben) return { ...this.dongGoi(ben), cached: true };
    }
    ...
    return { ...this.dongGoi(saved), cached: false };
  }
  ...
  private async thuThapDuLieu(
    ...
    //
    // ⚠️ `StatsService`'s `from`/`to` nhận CHUỖI `yyyy-mm-dd` (để tự nới trọn ngày theo múi
    // giờ user), không nhận `Date`. `epRange` ở đây luôn đã nằm đúng biên ngày (từ
    // `resolvePeriod`/`shiftRange`) nên format qua `formatDateOnly()` rồi parse lại cho
    // ĐÚNG khoảnh khắc ban đầu, không lệch múi giờ.
    const q = epRange
      ? {
          from: formatDateOnly(epRange.start, user.timezone),
          to: formatDateOnly(epRange.end, user.timezone),
          periodKind: period,
        }
      : { period };
